...
pub async fn run() {
    ...
    let (minecraft_ws_api, minecraft_api_runner, minecraft_api_canceller) =
        ws_protocol::WsClient::<minecraft::client::Api>::new();
    let (minecraft_ws_handler, minecraft_handler_canceller) =
        ws_protocol::WsClientHandler::new(minecraft::server::Handler::new(minecraft_ws_api));
    let (valheim_ws_api, valheim_api_runner, valheim_api_canceller) =
        ws_protocol::WsClient::<valheim::client::Api>::new();
    let (valheim_ws_handler, valheim_handler_canceller) =
        ws_protocol::WsClientHandler::new(valheim::server::Handler::new(valheim_ws_api));
